...
2.  **Verificación de acceso a la red:** Para comprobar la conectividad al servidor y al puerto (IP del computador) donde se encuentra la base de datos PostgreSQL.
    ...
    -   El enunciado *TRUE* indica que sí puede acceder desde su computadora al puerto en el que se encuentra ubicada la base de datos. La conexión está lista para iniciar el proceso de vinculación a la base de datos.
        ...
        ![](assets/Probar conexión al servidor.png)
...
3.  **Archivo pg_service.conf:**
    ...
    -   En la carpeta creada, guarde el archivo *pg_service.conf* enviado desde la oficina principal. Este contiene información referente a la base de datos y reconocerá los datos consignados durante la creación e ingreso a esta:

        ![](assets/Carpeta postgresql con el archivo conf.png)
...
5.  **Creación de la conexión a PostgreSQL**
    ...
    Finalmente Click en *Probar conexión*, en caso de ser exitosa, podrá observar el enunciado del éxito de la conexión (ver imagen EXITO) y enseguida diríjase a la sección **Ingreso a la base de datos**, en caso contrario, continúe en esta sección.
    ...
    *TcpTestSucceeded:* *True* incica que la configuración es correcta para aceptar conexiones.
    ...
    Una vez se haya establecido la conexión a la base de datos con éxito, ingrese a la base de datos de la siguiente manera: en la *Barra de* *Herramientas* busque *Base de datos,* y en el menú desplegado, seleccione *Administrador de bases de datos,* una herramienta muy útil para conectarse, explorar (ver tablas, esquemas o capas espaciales) y administrar (ejecutar consultas SQL e importar o exportar datos) diferentes bases de datos espaciales como PostGIS*:*

    ![](images/paste-4.png)

    Se abrirá una nueva ventana denominada *Administrador de BBDD,* en el cual se encuentra una barra de menús conformada por *Base de datos, Esquema, Tabla* y *Schema*. La barra de herramientas cuenta con la opción de *Actualizar*, *Ventana SQL*, *Importar capa/archivo* e *Exportar a archivo*. En el panel lateral derecho se encuentra *Proveedores* con el listado de los distintos tipos de bases de datos que QGIS puede manejar, seleccione *PostGIS* y en el menú desplegado observe las bases de datos y los esquemas que las conforman. Un esquema es considerado como una "carpeta" en la que se encuentran las tablas (.CSV, .XLSX, .XLS) y capas que conforman las bases de datos, permitiendo la administración de esta de una manera organizada. Asimismo, en el panel derecho encontrará la pestaña *Info*, donde se encuentra la información referente a la conexión, base de datos, esquema o capa seleccionada; *Tabla* para visualizar los datos de la tabla o capa seleccionada en formato tabular (como una hoja de Excel); y *Vista preliminar* para previsualizar espacialmente el contenido en caso de tener geometrías.
